...
isolation. Still gated by
`settings.EVILFLOWERS_DATAVERSE_AUTO_WHITELIST_WORKFLOW_RESUME`
...
def ensure_workflow_resume_ip_allowed(client: DataverseClient, resume_base_url: str) -> None:
    ...
    `settings.EVILFLOWERS_DATAVERSE_AUTO_WHITELIST_WORKFLOW_RESUME` is true.

    No-op otherwise. The legacy default was on; this proposal flips it
    off so the privilege-escalation primitive is operator-opt-in.
    """
    if not settings.EVILFLOWERS_DATAVERSE_AUTO_WHITELIST_WORKFLOW_RESUME:
        return
