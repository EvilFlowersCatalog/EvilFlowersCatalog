...
class AcquisitionDetail(SecuredView):
    ...
    def _get_acquisition(
        request, acquisition_id: UUID, checker: str = "check_catalog_manage"
    ) -> Acquisition:
        try:
            acquisition = Acquisition.objects.select_related("entry__catalog").get(
                pk=acquisition_id
            )
    ...
    def get(self, request, acquisition_id: UUID):
        acquisition = self._get_acquisition(
            request, acquisition_id, "check_catalog_read"
        )

        return SingleResponse(
            request, AcquisitionSerializer.Detailed.model_validate(acquisition)
        )
