...
class S3Storage(Storage):
    ...
    def _open(self, name, mode="rb"):
        return File(self._client.get_object(settings.STORAGE_S3_BUCKET, name))
    ...
    def _save(self, name, content):
        ...
        s3_object = self._client.put_object(
            settings.STORAGE_S3_BUCKET, name, content, content.getbuffer().nbytes, content_type=content_type
        )
    ...
    def delete(self, name):
        self._client.remove_object(settings.STORAGE_S3_BUCKET, name)
    ...
    def size(self, name):
        s3_object = self._client.stat_object(settings.STORAGE_S3_BUCKET, name)
